...
class UpdateLandlordInformationForm(forms.ModelForm):
    class Meta:
        model = LandlordInformation
        fields = ['first_name','last_name','phone_number', 'location', 'profile_picture', 'government_id']
        widgets = {
            
            'first_name': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'e.g. Ahmed'}),
            'last_name': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'e.g. Ibrahim'}),
            'phone_number': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'e.g. 08012345678'}),
            'location': forms.Select(attrs={'class': 'form-select'}),
            'profile_picture': forms.FileInput(attrs={'class': 'form-control'}),
            'government_id': forms.FileInput(attrs={'class': 'form-control'}),
